...
import {
  useAuthStore,
  useCategoryStore,
  useCategoryStore as useCategoryStoreDirect,
} from "@/store";
...
export default function SelectCategoriesScreen() {
  ...
  // Delete auto-created default categories when entering onboarding (only once per session)
  // Only delete if categories match the default ones from the database trigger
  useEffect(() => {
    if (!user?.id || hasClearedDefaults) {
      return;
    }

    const clearDefaults = async () => {
      try {
        // Fetch categories first to see what we have
        await fetchCategories(user.id);
        const currentCategories = useCategoryStoreDirect.getState().categories;

        // Default category names from the database trigger
        ...
        // Only delete categories that match the default ones (likely auto-created by trigger)
        // This prevents deleting user-created categories if they navigate back
        const categoriesToDelete = currentCategories.filter((cat) =>
          defaultCategoryNames.includes(cat.name)
        );

        if (categoriesToDelete.length > 0) {
          const deletePromises = categoriesToDelete.map((cat) =>
    ...
    clearDefaults();
  }, [user?.id, hasClearedDefaults, fetchCategories, deleteCategory]);
