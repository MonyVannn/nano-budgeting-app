...
import { ArrowLeft, ChevronRight, Lightbulb, Plus } from "lucide-react-native";
...
export default function AllocateBudgetScreen() {
  ...
  const handleContinue = async () => {
    ...
    const hasBudgets = categories.some(
      (c) => parseFloat(c.budget) > 0
    );
    if (!hasBudgets) {
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);
      Alert.alert(
    ...
    >
      ...
      >
        ...
        <View style={styles.suggestionBox}>
          ...
          <Pressable onPress={() => {}}>
            <Text style={styles.suggestionLink}>Suggest amounts →</Text>
          </Pressable>
        </View>
